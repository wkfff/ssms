...
CREATE OR REPLACE VIEW V_GRADE_R_D AS
	...
	LEFT JOIN SSM_GRADE_R_D T3 ON T1.SID=T3.R_DID
	ORDER BY T1.N_INDEX;$$

/*评审列表*/
CREATE OR REPLACE VIEW V_GRADE_R_M AS
	SELECT T1.SID,T2.C_TITLE,IFNULL(T3.C_NAME,'佚名')  C_NAME ,
	T3.P_PROVINCE,T3.P_CITY,T3.P_COUNTY FROM SSM_GRADE_R_M T1
	INNER JOIN ssm_grade_e_m t2 ON t1.r_eid=t2.sid
	INNER JOIN SYS_TENANT_E T3 ON T2.R_TENANT=T3.SID;$$
