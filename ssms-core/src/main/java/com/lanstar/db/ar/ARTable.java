...
import com.lanstar.db.dialect.IRowCallback;
...
public class ARTable extends ActiveRecordAbstr {
    ...
    public JdbcRecord query(){
        ...
        try{
            return dialect.query( session, st.sql, st.params );
    ...
    public JdbcRecordSet queryList(){
        SqlStatement st = TableStatementBuilder.query( this );
        try{
            return dialect.queryList( session, st.sql, st.params );
        } catch(SQLException e){
            throw new ActiveRecordException( e );
        }
    }
     ...
     * @param row 行处理回调函数
     */
    public void queryList( IRowCallback row ){
        SqlStatement st = TableStatementBuilder.query( this );
        try{
            dialect.queryList( session, st.sql, st.params, row );
        } catch(SQLException e){
            throw new ActiveRecordException( e );
        }
    }
    ...
    public int insert(){
        ...
        try{
            return dialect.executeUpdate( session, st.sql, st.params );
        } catch(SQLException e){
            throw new ActiveRecordException( e );
        }
    }
