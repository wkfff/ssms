 ...
 * 文件名称：ActionDoRenderResolver.java
 * 创建时间：2015-04-26
 * 创建用户：张铮彬
 */
...
public class ActionDoRenderResolver implements RenderResolver {
    @Override
    public Render getRender( ViewAndModel vam, RequestContext requestContext ) {
        return RenderFactory.me().getActionDoRender( vam.getModel(), requestContext );
    }
    ...
    public Render getErrorRender( HandleException e, RequestContext requestContext ) {
        ...
        Render render = RenderFactory.me().getActionDoRender( bean, requestContext );
