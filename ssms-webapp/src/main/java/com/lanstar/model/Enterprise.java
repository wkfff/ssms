...
public class Enterprise extends Model<Enterprise> {
    ...
    public EnterpriseUser getUser( String username, String password ) {
        return EnterpriseUser.dao.findFirst(
                "SELECT * FROM SYS_TENANT_E_USER WHERE UPPER(C_USER) = ? AND UPPER(C_PASSWD) = ? AND R_SID = ?",
                username.toUpperCase(), password.toUpperCase(), getStr( "SID" ) );
    }

    public static Enterprise findByCode( String code ) {
        return dao.findFirst( "SELECT * FROM SYS_TENANT_E WHERE UPPER(C_CODE)=?", code.toUpperCase() );
    }
}
