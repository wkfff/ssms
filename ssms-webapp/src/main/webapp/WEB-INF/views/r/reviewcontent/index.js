function ViewModel(recId) {
    ...
    model.noComplete.subscribe(function (newValue) {
        var url = '/r/reviewcontent/list?R_SID='+recId;
    ...
    var events = {
            recEvents: {
                saveClick: function () {
                    ...
                    $.getJSON("/r/reviewplan/check", {sid:recId}, function (data) {
                        if (data && data.N>0) {
                            ...
                            $.messager.confirm("完成确认", "您确认完成当前的评审吗？", function (action) {
                                if (action) {
                                    $.getJSON("/r/reviewplan/complete", {sid:recId}, function (data) {
                                        if (data.result == "OK") {
    ...
    var settings = {
        gridSettings: {
            //title:'评审内容',
            url: '/r/reviewcontent/list?R_SID='+recId,
            updateUrl:'/r/reviewcontent/save',
            idField: 'SID',
            rownumbers: false,
            pagination: false,
            singleSelect: true,
