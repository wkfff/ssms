...
<script type="text/javascript">
    ...
    function doSave(){
        ...
        $('#formMain').form('submit', {
            ...
            success: function(data){
                ...
                window.location.href='draft_rec?sid='+$str.replaceAll(data,'"','');
            }
        });
    }
    ...
    function doComplete(){
        $.get("check", {sid:'${sid!}'}, function (data) {
            if (data && data>0) {
                ...
                $.messager.confirm("完成确认", "您确认完成当前的自评吗？", function (action) {
                    if (action) {
                        $.get("complete", {sid:'${sid!}'}, function (data) {
                            if (data == "true" || data== "\"\"") {
    ...
    function doShow(v){
        ...
        $('#dg').datagrid({url:'/e/grade_d/list?R_SID=${sid!-1}'+(chk?'&score=0':'')});
    }
    function doReport(){
        window.location.href='report_rec?sid=${sid!}';
    }
    ...
    $(function () {
        $('#formMain').form('load','rec?sid=${sid!}&json');
        $('#dg').datagrid({
            title:'自评内容',
            url: '/e/grade_d/list?R_SID=${sid!-1}',
