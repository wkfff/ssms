...
(function () {
    ...
    ko.bindingHandlers.comboboxText = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            value = valueAccessor();
            if (value() == null) {
                curValue = $(element).combobox('getText');
                if (curValue) {
                    value(curValue);
                }
            }
            ...
            refreshValueFun = function (oriFun) {
                return function () {
                    value($(element).combobox('getText'));
                    return oriFun != null ? oriFun.apply($(element), arguments) : void 0;
                };
            };
            options.onSelect = refreshValueFun(options.onSelect);
    ...
    ko.bindingHandlers.numberboxValue = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            refreshValueFun = function (oriFun) {
                return function () {
                    value($(element).numberbox('getValue'));
                    return oriFun != null ? oriFun.apply($(element), arguments) : void 0;
                };
            };
            $(element).numberbox({
                onChange: refreshValueFun(options.onChange)
            });
        },
        update: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            var value;
            value = ko.utils.unwrapObservable(valueAccessor());
            if ($(element).numberbox('getValue') !== value) {
                return $(element).numberbox('setValue', value);
            }
        }
    };
    ...
    ko.bindingHandlers.textboxValue = {
        ...
        update: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            var value;
            value = ko.utils.unwrapObservable(valueAccessor());
            if ($(element).textbox('getValue') !== value) {
                return $(element).textbox('setValue', value);
            }
        }
    };
